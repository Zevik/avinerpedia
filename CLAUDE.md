...
Playwright starts `npm run dev` itself (or reuses a running server). **Check that port 3000 is really this app** (`curl -s localhost:3000 | grep אבינרפדיה`): `reuseExistingServer` happily tests another project's dev server on that port (happened 2026-09-27); if it's taken, start this app on another port and use `E2E_BASE_URL=http://localhost:<port>`. `E2E_PROD=1` runs against `npm run start` instead (build first). **`E2E_BASE_URL=https://avinerpedia.vercel.app npm run test:e2e` runs the suite against the deployed site** — the best post-deploy check. It uses 2 workers: full parallelism floods the live site with prefetches (cold functions) and navigations time out; in normal use they take 0.2–2 s. Output goes to `playwright-report/` and `test-results/` (gitignored). **Specs import `test`/`expect` from `tests/e2e/fixtures.ts`**, whose `page.goto` waits until every link in `<main>` is hydrated (React's `__reactProps$…` on the node): under the parallel suite's CPU load a click that landed mid-hydration was lost, which was the cause of the long-standing intermittent "clicked, URL never changed" failures (with prefetch floods, fixed by `prefetch={false}` on crowded pages).
...
- **`content_items`** — one row per page. Columns: `id`, `title` (unique; the import upserts on it), `main_category` (text the pages filter on: `סרטונים` / `מאמרים` / `שו"ת הלכה` / `סדרות`; the French lessons (`Cours en Français`, content_type `french`) were deleted on 2026-09-26, together with 14 more French items that had been filed as articles/videos (ids 20, 25, 27, 31, 33, 45, 47, 49, 55–60) — backups in `support/derived/backup/french-delete-2026-09-26.json` and `french-delete-2-2026-09-26.json`; the MDX files are still in `content/wiki`, so a re-import would bring them back), `sub_category` (specific topic or series name), `main_category_id` / `sub_category_id` (→ `categories`), `video_id`, `publish_date`, `summary`, `content_md`, `original_tags`, `is_active`, `created_at`, `updated_at` (trigger), `fts` (generated tsvector). Taxonomy columns from 002: `source_page_id` (unique, source wiki page id), `content_type` (`video|article|qa|series|french`), `root_topic`, `series_id` (→ `series`), `series_order`, `source_updated_at`.
...
| `/about`, `/rav-aviner`, `/accessibility`, `/privacy` | Static text pages (`components/legal/LegalPage.tsx`; the biography's text is Markdown in `lib/pages/rav-aviner.ts`, with schema.org Person JSON-LD), linked from the footer; the navbar's accessibility icon (next to search, same style — no floating widget) links to `/accessibility` |
...
- **`docs/topic-merge-review.csv`** (2026-09-26): every source category with 3+ items → what happened to it in the tree (kept / renamed / merged / moved to another core topic / became a source or series), live counts, the source's pages per type, what it was merged with, a suggestion (`לפצל לתת-נושא` / שם נרדף / להשאיר) and a `החלטה` column for the owner. Pending decisions; applying them means new third-level nodes (and search aliases) in `draft-topic-taxonomy.mjs` + the mapping.
- **Topics set in the admin form** (`topics_manual = true`, migration 007) are kept by `apply-topic-taxonomy.mjs --apply`: it re-inserts those items' links and leaves their primary node and `sub_category` alone; everything else is rebuilt from the source data.
...
- **Pages without query parameters are ISR** (`export const revalidate = 86400`): `/`, `/series`, `/topics`, and `/content/[id]`, `/series/[id]` (with an empty `generateStaticParams`, rendered on first visit). They are served from Vercel's CDN (`s-maxage=86400`). Don't add `force-dynamic` back; pages that read `searchParams` are dynamic on their own.
...
- **Owner facts are in `lib/site-info.ts`** (operator, contact e-mail, accessibility contact — זאב אבינר, no phone —, last-updated date of /about /privacy /accessibility; the site has no physical service point, so the statement has no physical-arrangements section; server regions and log retention are described generally on /privacy, with a link to Vercel's policy, rather than as exact values). While a value is `null`, the pages show a highlighted `[נדרש אישור בעל האתר: …]`. Fill them in before the public launch; whether the accessibility regulations (and their exemptions) and privacy duties apply to the operator is for the owner/legal advice to confirm.
...
| `/קטגוריה:X`, `/Category:X` | 301 → the curated node page (`/topics/הלכה/כשרות ומזון`), `/series/[id]` or a hub (`/videos`, `/qa`...; the deleted French category → `/`) |
...
Gotchas:
- **`import-from-wiki.ts` upserts on `title`**, and the stored titles no longer match the MDX titles (suffixes removed): a re-import would create duplicates. Match on `source_page_id` instead before using it again.
...
- **Titles: the source wiki's type suffixes are gone** — "(מאמר)", "(וידאו)", "(שו"ת)" were removed from the stored titles on 2026-09-25 (`scripts/source/strip-title-suffixes.mjs`, dry run / `--apply` with backup; 4,419 titles). `title` is unique, so hidden items (wiki redirect aliases) that held a clean title became `<title> [#id]`, and 47 pairs of active items that would share a title (an article and a video of the same name…) kept their suffixes — listed in `docs/title-conflicts-review.csv` for a decision (rename one in the admin form). `displayTitle()` (`lib/utils.ts`) still hides those suffixes on the site; use it wherever an item title is rendered. Redirects from old wiki URLs are keyed by source page id, so they are unaffected.
...
- **Not-found pages return HTTP 404** (with a `noindex` meta tag). Until 2026-09-25 the root `app/loading.tsx` streamed every page, so `notFound()` answered 200; it was removed together with the rare stuck client navigations it caused (see below).
...
- Pages: dashboard (counts), content list (filter, change main/sub category, **+ הוספת פריט**), content add/edit (`components/admin/ContentForm.tsx`, one form for `/admin/content/new` and `/admin/content/edit/[id]`): title, type (מאמר / וידאו / שו"ת — sets `content_type` and `main_category`; series episodes and French lessons keep theirs), **source** (dropdown from `sources`), **topics** (`components/admin/TopicPicker.tsx`: chips with ★ for the primary topic, search over all levels, the tree with checkboxes; ancestors are implied — `lib/topic-selection.ts`; saved with `admin_set_item_topics`; every save recounts the item's topics), date, summary, **video for any type** (a YouTube URL or id, or `Meir:<id>`, normalized by `normalizeVideoInput()` with a thumbnail preview — a Q&A with a video counts as both שו"ת and וידאו in the library), body, tags, active. A duplicate title (unique) shows an error. categories (legacy `categories` tree: create/rename/delete/merge).
...
- **Admin taxonomy editing** (rest): series + episode number in the form; replace the legacy categories screen with topic-tree and series management. (Type, source and topics are in the form.)
