...
- **Renaming a node**: rename it in `draft-topic-taxonomy.mjs` and regenerate, then `node scripts/source/rename-filter-node.mjs "<old path>" "<new path>" --apply` **before** `apply-topic-taxonomy.mjs --apply` — the apply upserts by path, so without the in-place rename the node gets a new id and `?topic=<id>` links break. Add the old name to `RENAMED_TOPICS` in `next.config.ts` (301 for `/topics/<old>/...`). Done for מועדים → חגים ומועדים (2026-09-25).
