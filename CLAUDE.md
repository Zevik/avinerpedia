...
- **Owner facts are in `lib/site-info.ts`** (operator, privacy contact e-mail, accessibility contact — זאב אבינר, no phone —, server regions, log retention; the site has no physical service point, so the statement has no physical-arrangements section). While a value is `null`, the pages show a highlighted `[נדרש אישור בעל האתר: …]`. Fill them in before the public launch; whether the accessibility regulations (and their exemptions) and privacy duties apply to the operator is for the owner/legal advice to confirm.
