...
- **Titles are displayed through `displayTitle()`** (`lib/utils.ts`), which drops the source wiki's "(מאמר)" suffix (2,345 titles). The stored title keeps it: `title` is unique and 77 items would collide without the suffix, and it matches the source. Use `displayTitle()` wherever a new place renders an item title (cards, headings, metadata, alt text).
